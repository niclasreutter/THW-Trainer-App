...
@section('content')
    <div class="max-w-7xl mx-auto p-6">
        ...
        <div class="mb-12 bg-white rounded-lg shadow-md p-6">
            ...
            <!-- Spielfortschritt -->
            <div class="mb-6 flex flex-wrap gap-4 justify-center md:justify-start">
                <!-- Streak -->
                <div class="flex items-center bg-gradient-to-r from-orange-100 to-red-100 rounded-lg px-4 py-2">
                    <div class="text-2xl mr-2">🔥</div>
                    <div>
                        <div class="text-lg font-bold text-orange-800">{{ $user->streak_days ?? 0 }}</div>
                        <div class="text-xs text-orange-600">Tage Streak</div>
                    </div>
                </div>

                <!-- Level -->
                <div class="flex items-center bg-gradient-to-r from-yellow-100 to-orange-100 rounded-lg px-4 py-2">
                    <div class="text-2xl mr-2">⭐</div>
                    <div>
                        <div class="text-lg font-bold text-yellow-800">Level {{ $user->level ?? 1 }}</div>
                        @php
                            $levelUpPoints = 100 * pow(1.5, ($user->level ?? 1) - 1);
                            $currentProgress = ($user->points ?? 0) % $levelUpPoints;
                        @endphp
                        <div class="text-xs text-yellow-600">{{ $currentProgress }}/{{ $levelUpPoints }} XP</div>
                    </div>
                </div>

                <!-- Daily Challenge -->
                <div class="flex items-center bg-gradient-to-r from-green-100 to-blue-100 rounded-lg px-4 py-2">
                    <div class="text-2xl mr-2">⚡</div>
                    <div>
                        <div class="text-lg font-bold text-green-800">{{ $user->daily_questions_solved ?? 0 }}/20</div>
                        <div class="text-xs text-green-600">Täglich</div>
                    </div>
                </div>

                <!-- Achievements -->
                <div class="flex items-center bg-gradient-to-r from-purple-100 to-pink-100 rounded-lg px-4 py-2">
                    <div class="text-2xl mr-2">🏆</div>
                    <div>
                        @php
                        ...
                        @endphp
                        <div class="text-lg font-bold text-purple-800">{{ $unlockedCount }}/{{ $totalAchievements }}</div>
                        <div class="text-xs text-purple-600">Achievements</div>
                    </div>
                </div>
            </div>
