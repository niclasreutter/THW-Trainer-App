    ...
    @if($question)
        ...
        <form method="POST" action="{{ route('lehrgaenge.submit', $question->lehrgang_slug) }}" id="practiceForm">
            ...
            <div class="mb-2 sm:mb-3 p-2 sm:p-3 border rounded-lg bg-gray-50 shadow-sm sm:hover:shadow-md sm:transition-shadow sm:duration-300">
                <div class="mb-1 text-[9px] sm:text-[10px] text-gray-500 flex items-center gap-1 flex-wrap">
                    <span>ID: {{ $question->id }}</span>
                    <span class="mx-0.5 sm:mx-1">&middot;</span>
                    <span>Lernabschnitt: {{ $question->lernabschnitt_name ?? ('Lernabschnitt ' . $question->lernabschnitt) }}</span>
                    <span class="mx-0.5 sm:mx-1">&middot;</span>
            ...
            @else
                @if(isset($currentSectionNr) && $currentSectionNr)
                <a href="{{ route('lehrgaenge.practice-section', ['slug' => $question->lehrgang_slug, 'sectionNr' => $currentSectionNr]) }}" class="w-full block text-center font-bold text-xs sm:text-base py-2.5 sm:py-2.5 px-4 rounded-lg no-underline transition-all duration-300" 
                   style="background-color: #1e3a8a; color: #fbbf24; box-shadow: 0 0 15px rgba(30, 58, 138, 0.3);"
