# Exam-History Redesign Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rewrite `exam-history.blade.php` to match the visual design of Dashboard/Bookmarks/Practice-Menu pages.

**Architecture:** Single-file view rewrite. No controller, route, or database changes. All data variables from `ExamController@history` remain identical. Page-specific CSS lives in `@push('styles')` within the blade file.

**Tech Stack:** Laravel Blade, CSS (page-scoped), Alpine.js (filter pills)

**Spec:** `docs/superpowers/specs/2026-03-17-exam-history-redesign-design.md`
...
|---|---|---|
| `resources/views/exam-history.blade.php` | Rewrite | Complete view template |

No new files. No modified files beyond this one.

---

## Chunk 1: Rewrite exam-history.blade.php

### Task 1: Scaffold — Header, Fonts, Container

**Files:**
- Modify: `resources/views/exam-history.blade.php` (complete rewrite)

**Reference files (read-only, for pattern matching):**
- `resources/views/practice-menu.blade.php` — header pattern, gami-pills, section-items, smart-action, stagger animation, fonts
- `resources/views/bookmarks/index.blade.php` — header pattern, gami-pills, smart-action, stagger animation
- `resources/views/exam-history.blade.php` — current version for data variables

- [ ] **Step 1: Read reference files**

Read these files to understand exact patterns:
- `resources/views/practice-menu.blade.php`
- `resources/views/bookmarks/index.blade.php`
- `resources/views/exam-history.blade.php` (current)

- [ ] **Step 2: Write the new blade file — scaffold with header + gami-pills**

Replace the entire content of `resources/views/exam-history.blade.php` with:
...
<style>
    /* Page-specific styles will be added in subsequent steps */
</style>
@endpush
...
<div class="dash-container">
    ...
    <!-- Content sections added in next steps -->
...
Open `/exam-history` in browser. Verify header and gami-pills render correctly in dark and light mode.

- [ ] **Step 4: Commit scaffold**

```bash
git add resources/views/exam-history.blade.php
git commit -m "🎨: Exam-History Scaffold (Header + Gami-Pills)"
```

---

### Task 2: Empty State + Smart-Action + Trend-Chart

- [ ] **Step 1: Add page-specific CSS for trend chart**

In the `<style>` block inside `@push('styles')`, add:
...
- [ ] **Step 2: Add Empty State + Smart-Action + Trend-Chart HTML**

Replace `<!-- Content sections added in next steps -->` with:
...
@else
    <div class="space-y-4">
        ...
        <a href="{{ route('practice.section', $weakSections[0]) }}" class="smart-action">
        ...
        <!-- Remaining sections added in next steps -->
...
Run: `npm run build && php artisan view:clear && php artisan cache:clear`
Test empty state (if possible) and trend chart with data. Check dark/light mode.

- [ ] **Step 4: Commit**

```bash
git add resources/views/exam-history.blade.php
git commit -m "🎨: Exam-History Trend-Chart + Smart-Action + Empty State"
```

---

### Task 3: Sachgebiet-Analyse + Empfehlungen

- [ ] **Step 1: Add CSS for recommendations**
...
/* ─── Section Items (reuse existing but add light-mode overrides) ─── */
.eh-section-item {
    display: flex;
    align-items: center;
    gap: 0.625rem;
...
- [ ] **Step 2: Add Sachgebiet-Analyse + Empfehlungen HTML**

Replace `<!-- Remaining sections added in next steps -->` with:
...
<!-- Prüfungsliste added in next step -->
```

- [ ] **Step 3: Verify renders**

Run: `npm run build && php artisan view:clear && php artisan cache:clear`
Check section analysis and recommendations. Test with user that has exam data.

- [ ] **Step 4: Commit**

```bash
git add resources/views/exam-history.blade.php
git commit -m "🎨: Exam-History Sachgebiet-Analyse + Empfehlungen"
```

---

### Task 4: Prüfungsliste mit Filter-Pills

- [ ] **Step 1: Add CSS for filter pills and exam items**
...
- [ ] **Step 2: Add Prüfungsliste HTML with Alpine.js filter**

Replace `<!-- Prüfungsliste added in next step -->` with:
...
- [ ] **Step 3: Close the space-y-4 div**

Make sure `</div>` closes the `<div class="space-y-4">` after the Prüfungsliste block.
...
Test:
1. Filter-Pills wechseln (Alle/Bestanden/Durchgefallen)
2. Dark mode korrekt
3. Light mode korrekt
4. Mobile Ansicht (schmaler Browser)
5. Empty state bei User ohne Prüfungen
6. Stagger-Animation beim Laden

- [ ] **Step 5: Final commit**

```bash
git add resources/views/exam-history.blade.php
git commit -m "🎨: Exam-History Redesign komplett"
```
