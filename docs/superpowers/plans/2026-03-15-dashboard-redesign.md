# Dashboard Redesign "THW Operations" Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Komplett neues Dashboard (`/dashboard`) im "Command Center"-Stil: THW-Blau dominant, technische Grid-Ästhetik, eckigere Karten, keine collapsible Sections, Dark/Light Mode.

**Architecture:** Einzige geänderte Datei ist `resources/views/dashboard.blade.php` – komplette Neuerstellung. Alle Variablen kommen wie bisher aus der Route-Closure (`routes/web.php`) oder werden inline in `@php`-Blöcken berechnet. Keine Backend-Änderungen.

**Tech Stack:** Laravel 12 Blade, Tailwind CSS (über bestehende `app.css` Klassen), Alpine.js, Bootstrap Icons (`bi bi-*`), bestehende CSS-Variablen (`--thw-blue`, `--gold-start`, `--glass-bg` etc.)
...
|-------|--------|---------------|
| `resources/views/dashboard.blade.php` | **Komplette Neuerstellung** | Alle 10 Sektionen des neuen Dashboards |
| `routes/web.php` | Nur lesen, **nicht ändern** | Referenz für übergebene Variablen |
| `resources/css/app.css` | Nur lesen, **nicht ändern** | Referenz für bestehende CSS-Klassen |

---

## Variablen-Referenz

Aus Route-Closure `routes/web.php` übergeben:
- `$user` – Auth-User
- `$recentExams` – letzte Prüfungen (`ExamStatistic`)
- `$totalQuestions` – Gesamtanzahl Fragen
- `$spacedRepetitionDue` – Anzahl fälliger SR-Fragen (int)
- `$weeklyActivity` – Collection mit `date`, `count`, `correct` pro Tag
- `$sectionStats` – Collection mit `lernabschnitt`, `total`, `correct`
- `$streakFreezeStatus` – `['remaining' => int]`

Inline in `@php`-Blöcken berechnet (exakt wie aktueller View):
- `$total`, `$progress`, `$progressPercent`, `$progressData`, `$threshold`
- `$exams`, `$failedArr`, `$hasFailedQuestions`, `$canStartExam`
- `$circumference`, `$theoryOffset`, `$examOffset`
- `$enrolledLehrgaenge`, `$enrolledLernpools`
- `$userAchievements`, `$unlockedCount`, `$totalAchievements`
- `$userLeague`, `$leagueInfo`
- `$streakMinQuestions`, `$todayQuestions`, `$questionsRemaining`, `$streakAtRisk`
- `$activeLernsession` (umbenennung von `$__liveInstance`)
- `$daysLeft`, `$dailyTarget`, `$todayAnswered`
- `$userOV`, `$isAusbilder`, `$ovStats`
- `$thisWeekTotal`, `$thisWeekCorrect`, `$thisWeekRate`, `$lastWeekTotal`, `$lastWeekRate`, `$rateDiff`

---

## Chunk 1: CSS Foundation + Status-Strip + Alert-Banner

### Task 1: Datei leeren und CSS-Grundgerüst + @php-Blöcke aufsetzen

**Files:**
- Modify: `resources/views/dashboard.blade.php` (komplette Neuerstellung)

- [ ] **Step 1: Alten Inhalt durch neuen Dateikopf ersetzen**

Datei mit folgendem Inhalt neu anlegen (alle @php-Berechnungen + CSS-Grundgerüst):
...
git commit -m "🎨: Dashboard CSS Grundgerüst"
```

---

### Task 2: Status-Strip implementieren

**Files:**
- Modify: `resources/views/dashboard.blade.php`

- [ ] **Step 1: SVG-Gradienten + Leaderboard-Modal-Script + Status-Strip HTML einfügen**

Direkt nach dem `@endphp`-Block, vor dem schließenden `@endsection`-Platzhalter, folgenden Inhalt anhängen:
...
- [ ] **Step 2: `@endsection` und schließendes `</div>` ans Ende der Datei anhängen**
...
- [ ] **Step 3: Build + Cache clear**

```bash
npm run build && php artisan view:clear && php artisan cache:clear
```
Erwartung: Kein Fehler. Im Browser `/dashboard` aufrufen: Status-Strip soll oben erscheinen mit Name, Level, XP-Balken, Streak (gold), Punkte, Liga.

- [ ] **Step 4: Commit**

```bash
git add resources/views/dashboard.blade.php
git commit -m "🎨: Dashboard Status-Strip"
```

---

### Task 3: Alert-Banner implementieren

**Files:**
- Modify: `resources/views/dashboard.blade.php`

- [ ] **Step 1: Alert-Banner HTML nach dem Status-Strip einfügen**

Direkt nach dem Status-Strip-`</div>`-Block und vor dem `</div>{{-- .ops-container --}}` Block einfügen:
...
npm run build && php artisan view:clear && php artisan cache:clear
```
Erwartung: Seite lädt ohne Fehler. Alerts erscheinen nur wenn die jeweilige Bedingung erfüllt ist (zum Testen vorübergehend `session()->flash('error', 'Test')` in tinker setzen).

- [ ] **Step 3: Commit**

```bash
git add resources/views/dashboard.blade.php
git commit -m "🎨: Dashboard Alert-Banner"
```

---

## Chunk 2: Next-Step Hero Card + Stats Grid

### Task 4: Next-Step Hero Card implementieren

**Files:**
- Modify: `resources/views/dashboard.blade.php`

- [ ] **Step 1: Hero Card HTML nach den Alert-Bannern einfügen**
...
npm run build && php artisan view:clear && php artisan cache:clear
```
Im Browser `/dashboard` aufrufen. Hero Card soll in THW-Blau erscheinen mit dem passenden Text je nach Lernstatus. Ring soll korrekt gefüllt sein.

- [ ] **Step 3: Commit**

```bash
git add resources/views/dashboard.blade.php
git commit -m "🎨: Dashboard Next-Step Hero Card"
```

---

### Task 5: Stats Grid (Theorie + Prüfung) implementieren

**Files:**
- Modify: `resources/views/dashboard.blade.php`

- [ ] **Step 1: Stats Grid HTML nach der Hero Card einfügen**
...
npm run build && php artisan view:clear && php artisan cache:clear
```
Beide Karten sollen nebeneinander erscheinen mit THW-Blau Top-Border. Buttons zeigen den korrekten Status.

- [ ] **Step 3: Commit**

```bash
git add resources/views/dashboard.blade.php
git commit -m "🎨: Dashboard Stats Grid"
```

---

## Chunk 3: Countdown + Aktivitäts-Chart

### Task 6: Countdown-Strip implementieren

**Files:**
- Modify: `resources/views/dashboard.blade.php`

- [ ] **Step 1: Countdown-Strip nach dem Stats Grid einfügen**
...
npm run build && php artisan view:clear && php artisan cache:clear
```
Erwartung: Wenn Prüfungsdatum gesetzt → Countdown mit Tagen + Mini-Bar. Sonst → CTA-Strip mit gestrichelter Borderlinie.

- [ ] **Step 3: Commit**

```bash
git add resources/views/dashboard.blade.php
git commit -m "🎨: Dashboard Countdown-Strip"
```

---

### Task 7: Aktivitäts-Chart implementieren

**Files:**
- Modify: `resources/views/dashboard.blade.php`

- [ ] **Step 1: Aktivitäts-Chart nach dem Countdown-Strip einfügen**
...
npm run build && php artisan view:clear && php artisan cache:clear
```
Balken sollen in THW-Blau erscheinen (nicht Gold wie vorher). Heutiger Balken = helleres Blau mit leichtem Glow.

- [ ] **Step 3: Commit**

```bash
git add resources/views/dashboard.blade.php
git commit -m "🎨: Dashboard Aktivitäts-Chart"
```

---

## Chunk 4: Heatmap + Spaced Rep + Ausbilder + Lehrgänge

### Task 8: Heatmap + Spaced Rep implementieren

**Files:**
- Modify: `resources/views/dashboard.blade.php`

- [ ] **Step 1: Heatmap-Zeile nach dem Aktivitäts-Chart einfügen**
...
npm run build && php artisan view:clear && php artisan cache:clear
```
Heatmap zeigt 10 farbige Zellen (2/3 der Breite), Spaced Rep Widget rechts daneben.

- [ ] **Step 3: Commit**

```bash
git add resources/views/dashboard.blade.php
git commit -m "🎨: Dashboard Heatmap und Spaced Rep"
```

---

### Task 9: Ausbilder-Karte + Lehrgänge Grid + Leaderboard-Modal implementieren

**Files:**
- Modify: `resources/views/dashboard.blade.php`

- [ ] **Step 1: Ausbilder-Karte nach Heatmap-Zeile einfügen**
...
- [ ] **Step 2: Lehrgänge & Lernpools Grid einfügen**
    ...
    <div>
        ...
        <div class="kurs-grid">
            ...
            @foreach($enrolledLernpools->take(1) as $lernpool)
                @php
                    $lpSolved = auth()->user()->lernpoolProgress()
...
- [ ] **Step 3: Leaderboard-Modal vor schließendem `</div>{{-- .ops-container --}}` einfügen**
...
> **Hinweis:** Das `</div>{{-- .ops-container --}}` aus Task 2 muss entfernt und durch diesen Block ersetzt werden. Die Datei endet jetzt mit dem Leaderboard-Modal, dann den Script-Tags, dann `@endsection`.

- [ ] **Step 4: Build + vollständiger visueller Test**
...
Im Browser prüfen:
- `/dashboard` lädt ohne PHP-Fehler
- Alle 9 Sektionen sichtbar (je nach Nutzerstatus)
- Dark Mode: alle Karten korrekt transparent/gläsern
- Light Mode (mit `localStorage.setItem('theme','light')` in Konsole): Borders und Balken sichtbar
- Mobil (< 768px): alles eingespaltig, kein horizontales Scrollen

- [ ] **Step 5: Commit**

```bash
git add resources/views/dashboard.blade.php
git commit -m "🎨: Dashboard Lehrgänge, Ausbilder, Leaderboard-Modal"
```

---

## Chunk 5: Abschluss & Qualitätsprüfung

### Task 10: Finale Prüfung und Bereinigung

**Files:**
- Modify: `resources/views/dashboard.blade.php` (ggf. kleine Fixes)

- [ ] **Step 1: Alle Zustände der Hero Card manuell durchgehen**

Prüfe in tinker / DB direkt:
...
```php
// Zustand B simulieren: Fortschritt < 100%
// (Normalzustand für neue User – kein Eingriff nötig)

// Prüfe ob $canStartExam korrekt berechnet wird
$user = \App\Models\User::find(YOUR_USER_ID);
$progress = \App\Models\UserQuestionProgress::countMastered($user->id);
$total = \App\Models\Question::count();
$failedArr = is_array($user->exam_failed_questions) ? $user->exam_failed_questions : json_decode($user->exam_failed_questions, true) ?? [];
$canStart = $progress >= $total && count($failedArr) === 0;
echo "Progress: $progress/$total, CanStart: " . ($canStart ? 'YES' : 'NO');
```

- [ ] **Step 2: Light Mode visuell prüfen**
...
location.reload();
```
Prüfen: XP-Balken sichtbar, Borders sichtbar, Heatmap "none"-Zellen sichtbar, kein unsichtbarer weißer Text auf weißem Hintergrund.
...
- [ ] **Step 3: Mobile Breakpoints prüfen**

DevTools auf 375px (iPhone SE) stellen. Prüfen:
- Kein horizontales Scrollen
- Status-Strip zweizeilig (Name + Stats untereinander)
- Hero Card einspaltig
- Stats-Grid einspaltig
- Heatmap + Spaced Rep gestapelt
- Kurs-Grid einspaltig

- [ ] **Step 4: Finaler Commit**

```bash
git add resources/views/dashboard.blade.php
git commit -m "🎨: Dashboard Redesign THW Operations"
```

---

## Variablen-Checkliste (Quick Reference)

| Variable | Quelle | Fallback |
|----------|--------|---------|
| `$totalQuestions` | Route-Closure | `Question::count()` |
| `$recentExams` | Route-Closure | `collect()` |
| `$spacedRepetitionDue` | Route-Closure | `0` |
| `$weeklyActivity` | Route-Closure | `collect()` |
| `$sectionStats` | Route-Closure | `collect()` |
| `$streakFreezeStatus` | Route-Closure | `['remaining' => 0]` |
| `$progressPercent` | inline `@php` | `0` |
| `$progress` | inline `@php` | `0` |
| `$exams` | inline `@php` | `0` |
| `$canStartExam` | inline `@php` | `false` |
| `$daysLeft` | inline `@php` | `null` |
| `$dailyTarget` | inline `@php` | `null` |
| `$todayAnswered` | inline `@php` | `0` |
| `$activeLernsession` | inline `@php` | `null` |
| `$isAusbilder` | inline `@php` | `false` |
| `$ovStats` | inline `@php` | `null` |
